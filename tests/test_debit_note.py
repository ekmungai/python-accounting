...
from python_accounting.transactions import DebitNote
...
def test_debit_note_ledgers(session, entity, currency):
    """Tests debit note transaction ledger records"""
...
def test_debit_note_validation(session, entity, currency):
    """Tests the validation of debit note transactions"""
    account1 = Account(
        name="test account one",
    ...
    with pytest.raises(InvalidMainAccountError) as e:
        session.commit()
    assert str(e.value) == "DebitNote Transaction main Account be of type Payable."
    ...
    assert (
        str(e.value)
        == """DebitNote Transaction Line Item Account type must
