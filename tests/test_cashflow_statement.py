...
def test_cashflow_statement(session, entity, currency):
    """Tests the generation of an entity's cash flow statement"""
    ...
    statement = CashflowStatement(session)

    # # totals
    assert statement.totals["OPERATING_CASH_FLOW"] == 467.5
    assert statement.totals["INVESTMENT_CASH_FLOW"] == -100
    assert statement.totals["FINANCING_CASH_FLOW"] == 7
    assert statement.totals["NET_CASH_FLOW"] == 374.5

    # # balances
    assert statement.balances["OPERATING_CASH_FLOW"]["Net Profit"] == 265
    assert statement.balances["OPERATING_CASH_FLOW"]["Receivables"] == -22
    assert statement.balances["OPERATING_CASH_FLOW"]["Payables"] == 220
    assert statement.balances["OPERATING_CASH_FLOW"]["Taxation"] == 4.5
    assert statement.balances["INVESTMENT_CASH_FLOW"]["Non Current Assets"] == -100
    assert statement.balances["FINANCING_CASH_FLOW"]["Equity"] == 7
    assert statement.balances["NET_CASH_FLOW"]["Beginning Cash Balance"] == 100
    assert statement.balances["NET_CASH_FLOW"]["Net Cash Flow"] == 374.5

    # # results
    assert statement.result_amounts["END_CASH_BALANCE"] == 474.5
    assert statement.result_amounts["CASHBOOK_BALANCE"] == 474.5
